...
## 🗺️ Core Architecture & Concept Diagrams

Here are visual representations of the most critical enterprise Java and system design concepts.
...
graph TD
    subgraph JVM ["Java Virtual Machine (JVM)"]
        subgraph RuntimeMemory ["Runtime Data Areas"]
            Heap["Heap Area (Eden, S0/S1, Old Gen)"]
            Stack["Stack Area (Frames, Local Vars)"]
            Meta["Metaspace (Class Metadata)"]
            PC["PC Registers"]
            NativeStack["Native Method Stack"]
        end
        ClassLoader["Class Loader Subsystem"] --> RuntimeMemory
        RuntimeMemory --> ExecutionEngine["Execution Engine (JIT, Interpreter, GC)"]
    end
    SourceCode[".java File"] --> Compiler["Java Compiler (javac)"] --> Bytecode[".class File"] --> ClassLoader
```

### 2. HashMap Collision & Java 8+ Treeification
```mermaid
graph TD
    subgraph HashMap ["HashMap Table Array (Buckets)"]
        Bucket0["Bucket 0: null key"]
        Bucket1["Bucket 1 (Linked List)"]
        Bucket2["Bucket 2 (Treeified Node Array)"]
    end
    Bucket1 --> NodeA1["Node (Key A, Val A)"] --> NodeA2["Node (Key B, Val B)"]
    Bucket2 --> TreeNode["Red-Black Tree Root (O(log N))"]
    TreeNode --> LeftChild["Left Child"]
    TreeNode --> RightChild["Right Child"]
```

### 3. Spring Bean Lifecycle Flow
```mermaid
graph TD
    Start([Start]) --> Inst["1. Instantiation (Constructor)"]
    Inst --> DI["2. Dependency Injection (@Autowired)"]
    DI --> Aware["3. Aware Interface Callbacks"]
    Aware --> BPP_Before["4. BeanPostProcessor (Before Init)"]
    BPP_Before --> Init["5. Initialization (@PostConstruct / init-method)"]
    Init --> BPP_After["6. BeanPostProcessor (After Init / AOP Proxy)"]
    BPP_After --> Ready["7. Bean Ready for Use"]
    Ready --> Shutdown([Shutdown])
    Shutdown --> Destroy["8. Destruction (@PreDestroy / destroy-method)"]
```

### 4. Spring @Transactional AOP Proxy Interception
```mermaid
sequenceDiagram
    autonumber
    actor Client
    ...
    TM->>DB: Set AutoCommit(false) & Start TX
    TI->>Target: execute()
    alt Success
        Target-->>TI: return result
        TI->>TM: commit()
        TM->>DB: COMMIT
    else Exception
        Target-->>TI: throw Exception
        TI->>TM: rollback()
        TM->>DB: ROLLBACK
    end
    TI-->>Proxy: return
...
### 5. Saga Pattern (Distributed Transactions): Choreography vs Orchestration
```mermaid
graph TD
    subgraph Choreography ["Event-Driven Choreography"]
        OrderService1["Order Service"] -->|Order Created| Kafka["Kafka Topic"]
        Kafka -->|Consume Event| PaymentService1["Payment Service"]
        PaymentService1 -->|Payment Successful| Kafka
        Kafka -->|Consume Event| DeliveryService1["Delivery Service"]
    end
    subgraph Orchestration ["Orchestrator-Based"]
        OrderService2["Order Service"] -->|Start Saga| Orchestrator["Saga Orchestrator"]
        Orchestrator -->|1. Process Payment| PaymentService2["Payment Service"]
        PaymentService2 -->|Payment Success| Orchestrator
        Orchestrator -->|2. Create Delivery| DeliveryService2["Delivery Service"]
    end
```

### 6. Hibernate N+1 Query Problem Visualization
```mermaid
sequenceDiagram
    autonumber
    actor App as Spring Application
    participant DB as PostgreSQL Database

    App->>DB: SELECT * FROM ParentEntity (1 Query)
    DB-->>App: List of N ParentEntities
    loop For each ParentEntity (N times)
        App->>DB: SELECT * FROM ChildEntity WHERE parent_id = ? (N Queries)
        DB-->>App: Child details
    end
    Note over App,DB: Total Queries: 1 + N
```
